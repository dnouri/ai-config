---
name: excalidraw
description: Create hand-drawn-style Excalidraw diagrams (architecture, flowcharts, sequence) by writing .excalidraw JSON files with the write tool; optionally upload for a shareable excalidraw.com link. Use when asked to draw, diagram, or visualize architecture or processes.
license: MIT
compatibility: Python 3 with the cryptography package for the optional upload step (or uv). Everything else is plain JSON.
---
...
3. **Save the file** using the `write` tool to create a `.excalidraw` file
4. **Optionally upload** for a shareable link using `scripts/upload.py` via `bash`
...
Run the upload script (located in this skill's `scripts/` directory) via bash:

```bash
# Primary (works when the system python has the cryptography package)
python3 {baseDir}/scripts/upload.py ~/diagrams/my_diagram.excalidraw

# No-install fallback when cryptography is missing
uv run --with cryptography {baseDir}/scripts/upload.py ~/diagrams/my_diagram.excalidraw
```
...
*Adapted from the [Hermes Agent](https://github.com/NousResearch/hermes-agent) excalidraw skill (MIT).*
